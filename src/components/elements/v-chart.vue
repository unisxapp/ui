<template>
  <div class="chart_tab">
    <div class="chart-buttons">
      <button
        class="chart-button-item"
        :class="{ active: selectedRange === 'w' }"
        @click="filteredChartValues('w')"
      >
        W
      </button>
      <button
        class="chart-button-item"
        :class="{ active: selectedRange === 'q' }"
        @click="filteredChartValues('q')"
      >
        Q
      </button>
      <button
        class="chart-button-item"
        :class="{ active: !selectedRange }"
        @click="filteredChartValues()"
      >
        All
      </button>
    </div>
...
import {
  ethPromise,
  getFinancialContractProperties,
  getPosition,
} from "../..//core/eth";
...
export default {
  ...
  computed: {
    ...
    syntChartValues: function () {
      return this.filteredChart.length
        ? [...this.filteredChart.map((item) => +item.price)]
        : [];
    },
    syntChartKeys: function () {
      return this.filteredChart.length
        ? [...this.filteredChart.map((item) => item.date)]
        : [];
    },
  ...
  methods: {
    ...mapActions(["GET_HISTORICAL_PRICES"]),
    ...
    filteredChartValues(range = "") {
      ...
      if (range === "w") {
        ...
        const sortedObj = this.historicalPrices.filter(
          (k) => euroDate(k.date) >= monday
        );
      ...
      if (range === "q") {
        ...
        const sortedObj = this.historicalPrices.filter(
          (k) => euroDate(k.date) >= startQuarter
        );
        this.filteredChart = [...sortedObj];
...
button.chart-button-item {
  ...
  background: linear-gradient(
      0deg,
      rgba(21, 0, 153, 0.3008) -18.27%,
      rgba(255, 255, 255, 0.6272) 171.15%
    ),
    linear-gradient(0deg, #408cff, #408cff), #f9c02f;
  border-radius: 39px;
  color: #ffffff;
  transition: 0.3s;
}
button.chart-button-item.active {
  background: linear-gradient(
      0deg,
      rgba(94, 71, 235, 0.4418) -18.27%,
      rgba(255, 255, 255, 0.9212) 130.77%
    ),
    linear-gradient(0deg, #9672ff, #9672ff), #f9c02f;
}
</style>
